...
io.on("connection", function(socket){
    ...
    socket.on('team_data',function(name, team_data){
        ...
        if (playerCount % 2 == 1){
            ...
            info.user1.data = {id: socket.id, command: "", mega: false, megable: true, megaTxt: "メガ進化", Z: false, Zable: true, ZTxt: "Z技", ultra: false, ultrable: true, ultraTxt: "ウルトラバースト", dyna: false, dynable: false, dynaTxt: "ダイマックス", giga: false, gigable: true, gigaTxt: "キョダイマックス"}
            info.user1.con = {TN: name, p_con: "", f_con: "", used: "", log: ""}
            data.push(info)
            io.to(socket.id).emit("find enemy", {})
        } else {
            data[room_search(socket.id)].user2.data = {id: socket.id, command: "", mega: false, megable: true, megaTxt: "メガ進化", Z: true, Zable: true, ZTxt: "Z技", ultra: false, ultrable: true, ultraTxt: "ウルトラバースト", dyna: false, dynable: false, dynaTxt: "ダイマックス", giga: false, gigable: true, gigaTxt: "キョダイマックス"}
    ...
    socket.on("action decide", function(val, opt) {
        ...
        if (data[room].user1.data.id == socket.id){
            ...
            data[room].user1.data.ultra = opt.ultra
            ...
            data[room].user2.data.ultra = opt.ultra
