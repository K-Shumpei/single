...
// 全体の場へ効果をもたらす技
function all_field_status_move(atk, move){
    const atk_item = document.getElementById(atk + "_item").textContent
    for (let i = 0; i < all_field_status_move_list.length; i++){
        if (move[0] == all_field_status_move_list[i][0]){
            txt = all_field_status_move_list[i][2] + CR
            document.battle_log.battle_log.value += txt
            const move_type = all_field_status_move_list[i][3]
            if (move_type == "天候"){
                for (const j of ["A", "B"]){
                    let condition = document.battle[j + "_field_condition"].value
                    document.battle[j + "_field_condition"].value = ""
                    let turn = 5
                    const convert = [["にほんばれ", "あついいわ"], ["あまごい", "しめったいわ"], ["すなあらし", "さらさらいわ"], ["あられ", "つめたいいわ"]]
                    for (let k = 0; k < 4; k++){
                        if (move[0] == convert[k][0] && atk_item == convert[k][1]){
                            turn = 8
                        }
                    }
                    for (let k = 0; k < condition.split("\n").length - 1; k++){
                        if (!(condition.split("\n")[k].includes("にほんばれ") || condition.split("\n")[k].includes("あめ") || condition.split("\n")[k].includes("すなあらし") || condition.split("\n")[k].includes("あられ"))){
                            document.battle[j + "_field_condition"].value += condition.split("\n")[k] + CR
                        }
                    }
                    document.battle[j + "_field_condition"].value += all_field_status_move_list[i][1] + "　" + turn + "/" + turn + CR
                }
                if (move[0] == "あられ"){
                    for (const team of ["A", "B"]){
                        if (new get(team).ability == "アイスフェイス" && new get(team).name == "コオリッポ(ナイスフェイス)"){
                            txt = team + "チームの　" + new get(team).name + "の　アイスフェイス！" + CR
                            document.battle_log.battle_log.value += txt
                            form_chenge(team, "コオリッポ(アイスフェイス)")
                        }
                    }
                }
            } else if (move_type == "フィールド"){
                for (const team of ["A", "B"]){
                    condition_remove(team, "field", "フィールド")
                    let turn = 5
                    if (atk_item == "グランドコート"){
                        turn = 8
                    }
                    document.battle[team + "_field_condition"].value += all_field_status_move_list[i][1] + "　" + turn + "/" + turn + CR
                }
                for (const team of ["A", "B"]){
                    if ((move[0] == "エレキフィールド" && new get(team).item == "エレキシード") || (move[0] == "グラスフィールド" && new get(team).item == "グラスシード")){
                        rank_change_not_status(team, "B", 1, 100, new get(team).item)
                        set_recycle_item(team)
                    } else if ((move[0] == "サイコフィールド" && new get(team).item == "サイコシード") || (move[0] == "ミストフィールド" && new get(team).item == "ミストシード")){
                        rank_change_not_status(team, "D", 1, 100, new get(team).item)
                        set_recycle_item(team)
                    }
                }
                for (const team of ["A", "B"]){
                    if (new get(team).ability == "ぎたい"){
                        if (new get(team).f_con.includes("エレキフィールド")){
                            document.getElementById(team + "_type").textContent = "でんき"
                        } else if (new get(team).f_con.includes("グラスフィールド")){
                            document.getElementById(team + "_type").textContent = "くさ"
                        } else if (new get(team).f_con.includes("サイコフィールド")){
                            document.getElementById(team + "_type").textContent = "エスパー"
                        } else if (new get(team).f_con.includes("ミストフィールド")){
                            document.getElementById(team + "_type").textContent = "フェアリー"
                        }
                        txt = team + "チームの　" + new get(team).name + "　は　ぎたいでタイプが変わった" + CR
                        document.battle_log.battle_log.value += txt
                        if (new get(team).p_con.includes("もりののろい") && new get(team).type != "くさ"){
                            document.getElementById(team + "_type").textContent += "、くさ"
                        }
            ...
            } else if (move_type == "ルーム"){
                for (const team of ["A", "B"]){
                    if (new get(team).f_con.includes(move[0])){
                        condition_remove(team, "field", move[0])
                        if (move[0] == "マジックルーム"){
                            for (let i = 0; i < 3; i++){
                                if (document.getElementById(team + "_" + i + "_existence").textContent == "戦闘中"){
                                    document.getElementById(team + "_item").textContent = document.getElementById(team + "_" + i + "_item").textContent
                                }
                            }
                        }
                        if (move[0] == "トリックルーム" && new get(team).item == "ルームサービス"){
                            rank_change_not_status(team, "S", -1, 100, "ルームサービス")
                            set_recycle_item(team)
                        }
                    } else {
                        document.battle[team + "_field_condition"].value += move[0] + "　5/5" + CR
                        if (move[0] == "マジックルーム"){
                ...
                document.battle.A_field_condition.value += move[0] + CR
                document.battle.B_field_condition.value += move[0] + CR
            } else if (move[0] == "じゅうりょく"){
                for (const team of ["A", "B"]){
                    document.battle[team + "_field_condition"].value += "じゅうりょく　5/5" + CR
                    if (new get(team).p_con.includes("空を飛ぶ")){
                        condition_remove(team, "poke", "空を飛ぶ")
                        condition_remove(team, "poke", "溜め技")
                    }
                    condition_remove(team, "poke", "テレキネシス")
                    condition_remove(team, "poke", "でんじふゆう")
                }
            } else if (move[0] == "フェアリーロック"){
                document.battle.A_field_condition.value += all_field_status_move_list[i][1] + "　開始" + CR
                document.battle.B_field_condition.value += all_field_status_move_list[i][1] + "　開始" + CR
            }
        }
    }
}
